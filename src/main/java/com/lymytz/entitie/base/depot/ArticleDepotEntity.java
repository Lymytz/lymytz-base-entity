...
@Getter
@Setter
public class ArticleDepotEntity extends BaseEntity implements Serializable {
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    private DepotsEntity depotPr;
    @JoinColumn(name = "default_cond", referencedColumnName = "id")
    @ManyToOne(fetch = FetchType.LAZY)
    private ConditionnementArticleEntity defaultCond;
