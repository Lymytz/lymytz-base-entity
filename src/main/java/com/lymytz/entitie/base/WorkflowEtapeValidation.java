...
@Getter
@Setter
public class WorkflowEtapeValidation extends BaseEntity implements Serializable {
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    private WorkflowModelDoc documentModel;
    @JoinColumn(name = "etape_suivante", referencedColumnName = "id")
    @ManyToOne(fetch = FetchType.LAZY)
    private WorkflowEtapeValidation etapeSuivante;

    @OneToMany(mappedBy = "etapeValide")
    private List<WorkflowAutorisationValidDoc> autorisations;
}
